...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def getDash(self, params, forceRefresh=False):
		...
		if not 'dash' in data['result']:
			...
			try:
				expiresAt = int(self.regStr(reg=r'deadline=(\d+)', src=purl).group(1)) - 60
			except:
		...
		for video in dashinfos['video']:
			try:
				deadline = int(self.regStr(reg=r'deadline=(\d+)', src=video['baseUrl']).group(1))
			except:
		...
		for audio in dashinfos['audio']:
			try:
				deadline = int(self.regStr(reg=r'deadline=(\d+)', src=audio['baseUrl']).group(1))
			except:
				deadline = int(time.time()) + 600
			deadlineList.append(deadline)
